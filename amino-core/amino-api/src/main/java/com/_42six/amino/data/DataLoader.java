...
public interface DataLoader {
	...
	/**
	 * Determine if this DataLoader can read from the provided InputSplit.  This is used on enrichment jobs, and if not
     * implemented correctly, enrichment jobs that use this DataLoader will break.
     *
	 * If reading from HDFS, it is recommended that you cast the InputSplit to a FileSplit and call getPath() to see if
     * it matches the source file location. Unless both dataloaders are reading from the same HDFS directory, then you
     * should examine the rawRowOfData to see if it's the expected data.  If reading from Accumulo, you can examine the
     * rawRowOfData to see if it's the expected data.
     *
